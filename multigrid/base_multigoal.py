...
from .core.actions import ActionUpDown
...
class MultiGoalGridEnv(gym.Env, RandomMixin, ABC):
    """
    ...
    def __init__(
        ...
        self.hidden_goals = hidden_goals
    ...
    def observation_space(self):
        ...
        d['global'] = spaces.Dict({
            'num_goals': spaces.Discrete(len(self.goals)+1),
        })
        if not self.hidden_goals:
            d['global']['goals'] = spaces.Sequence(
                spaces.Tuple((spaces.Discrete(self.width), spaces.Discrete(self.height))),
                len(self.goals),
            )
    ...
    def step(
        ...
        # combine agent rewards
        infos = defaultdict(dict)
        t = self.step_count - 1
        cur_reward = np.mean(list(rewards.values()))
        if len(self.goals) == 0:
            cur_reward += (2 - t / self.max_steps) / (1 - self.decay) 
        self.total_rewards += (self.decay**t) * cur_reward
        infos['cur_reward'] = cur_reward
        infos['total_reward'] = self.total_rewards
    ...
    def gen_obs(self) -> dict[AgentID, ObsType]:
        ...
        for i in range(self.num_agents):
            observations[i] = {
                ...
                'location': self.agents[i].state.pos,
            }
        observations['global'] = {'num_goals': len(self.goals)}
        if not self.hidden_goals:
            observations['global']['goals'] = self.goals.copy()
        return observations
    ...
    def handle_actions(
        ...
        rewards : dict[AgentID, SupportsFloat]
            Reward for each agent
        """
        rewards = {agent_index: -1 for agent_index in range(self.num_agents)}
        ...
        for i in order:
            ...
            # Move left
            if action == ActionUpDown.left:
                fwd_pos = np.array(agent.state.pos)
                fwd_pos[0] -= 1
                if self.move(agent, fwd_pos, rewards):
                    continue
            
            # Move right
            elif action == ActionUpDown.right:
                fwd_pos = np.array(agent.state.pos)
                fwd_pos[0] += 1
                if self.move(agent, fwd_pos, rewards):
                    continue
            ...
            # Pick up an object
            elif action == ActionUpDown.pickup:
            ...
            elif action == ActionUpDown.toggle:
                fwd_pos = agent.front_pos
                fwd_obj = self.grid.get(*fwd_pos)
    ...
    def move(
        ...
        rewards):
        """
        Move all agents forward in the direction they are facing.
        """
        fwd_obj = self.grid.get(*fwd_pos)
    ...
    def on_goal(
        ...
        terminations):
        """
        Callback for when an agent reaches the goal.
        This method is called when an agent reaches the goal object.
        It updates the rewards and terminations dictionaries accordingly.
        """
        self.goals.remove(agent.state.pos)
        rewards[agent.index] = 1
        if agent.state.pos not in self.goals:
            self.put_obj(WorldObj.empty(), *agent.state.pos)
        if len(self.goals) == 0:
